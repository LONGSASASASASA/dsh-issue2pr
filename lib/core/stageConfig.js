...
import { readTriggerText, goalBriefOf } from "../stages/helpers.js";
...
export const STAGE_DEFS = {
  P1: {
    ...
    prompts: { "": "你是 IssueAnalyzer。把自然语言 Issue 提炼为结构化契约，只输出 JSON，不要输出其他文字。goal 是一句话北极星目标（全流水线只读注入的权威目标）：动词开头、含对象模块、含可判定完成状态，例「修复登录后刷新偶发退出的问题，使会话恢复测试通过」；non_goals 列出明确不处理的范围，没有则省略。" },
    caps: { route: true, exec: true, delegate: true },
    delegateSpec: { inputs: "trigger", output: "01-issue-analysis.json",
      contract: '{"goal":"一句话目标","phenomenon":"现象","trigger":"触发条件","scope":["影响模块"],"success_criteria":["可验证成功标准"],"constraints":["约束"],"non_goals":["明确不做什么"],"risk_level":"low|medium|high"}' },
  },
  P2: {
  ...
  P3: {
    ...
    delegateSpec: { inputs: ["goal", "02-search-candidates.json", "repo"], output: "03-code-understanding.md",
  ...
  P4: {
    ...
    caps: { route: true, exec: true, delegate: true },
    delegateSpec: { inputs: ["goal", "03-code-understanding.md"], output: "04-hypotheses.json",
  ...
  P5: {
    ...
    caps: { route: true, exec: true, delegate: true },
    delegateSpec: { inputs: ["goal", "04-hypotheses.json"], output: "05-task-graph.json",
  ...
  P6: {
    ...
    params: {
      ...
      dshTimeoutMin: { def: 120, type: "number", label: "dsh 智能体执行超时", unit: "分钟",
        hint: "p6Mode=dsh（DSH 原生智能体）无人值守执行任务包的最长等待；超时取消回收并回退等人工会话" },
    ...
    delegateSpec: { inputs: ["05-task-graph.json"], output: "06-implementation/",
      contract: "patches/*.diff（unified diff，仅 patched 节点生成）+ coder-report.json（tasks 覆盖 TaskGraph 全部节点，每项含 node、status=patched|no_change|failed；patched 含 patch 路径，no_change/failed 含 reason；全部完成后写报告）" },
  },
  P7: {
  ...
  P9: {
    ...
    delegateSpec: { inputs: ["goal", "06-implementation/coder-report.json", "07-test-report.json"], output: "08-review-report.json",
      contract: '{"diff_scope":"结论","api_security":"结论","tests":"结论","verdict":"pass|fail"}' },
  },
  P10: {
    name: "FailureClassifier", desc: "失败旁路 · 仅失败时执行",
    prompts: { "": "你是 FailureClassifier。依据本轮执行报告、原始日志和真实历史记录分类并给出处理路径。类别为实现错误/根因错误/测试选择/环境缺失/权限被拒/反复失败/执行超时/执行取消/原因未确定。必须保留程序记录的超时、取消、启动失败等事实；超时不等于实现错误。证据不足归为原因未确定，只有历史记录证明相同问题多次发生才能归为反复失败，不得从缺少信息推断反复失败。detail 引用具体证据；只输出 JSON。" },
    caps: { route: true, exec: true, delegate: true },
    delegateSpec: { inputs: "failure", output: "09-failure-analysis.json",
      contract: '{"category":"类别（实现错误/根因错误/测试选择/环境缺失/权限被拒/反复失败/执行超时/执行取消/原因未确定）","detail":"本轮报告与日志依据；反复失败必须引用真实历史记录","action":"replan|rollback|escalate"}' },
  },
  P11: {
    name: "PRBuilder + Eval", desc: "PR 说明 + Gate 评测 · 复核门",
    prompts: {
      desc: "你是 PRBuilder。生成忠实反映修改与验证过程的 PR 说明（中文 markdown）：背景/根因/修改点/验证证据/风险。禁止夸大。",
      gate: "你是 EvaluationRunner。依据真实产物评测 ROOT 根因有证据、DESC 说明忠实、ACCEPT 需求与约束闭合。PATCH 应用、TEST 测试、DIFF 审查由流水线证据确定，不从 PR 说明重新猜测。失败必须在 reasons 对应项给出引用文件和具体原因；证据不足不能声称通过。只输出 JSON。",
    },
    caps: { route: true, exec: true, delegate: true },
    delegateSpec: { inputs: ["01-issue-analysis.json", "04-hypotheses.json", "08-review-report.json", "07-test-report.json", "ledger/patch-ledger.jsonl", "06-implementation/coder-report.json", "repo"], output: "10-pr-description.md",
      contract: "10-pr-description.md（忠实于真实补丁的中文 PR 说明）+ 11-eval-report.json（ROOT/PATCH/TEST/DIFF/DESC/ACCEPT 六项 pass|fail；reasons 按门禁记录引用文件和具体失败原因）。依据实际补丁、应用账本、测试与审查报告判定，全部 pass 才可放行。" },
  },
};
...
export function validateStageConfig(sc) {
  ...
  for (const [id, cfg] of Object.entries(sc)) {
    ...
    const hasProvider = typeof cfg.provider === "string" && cfg.provider.trim() !== "";
    const hasModel = typeof cfg.model === "string" && cfg.model.trim() !== "";
    if (hasProvider !== hasModel) return [false, `stageConfig.${id} 的 provider 与 model 必须成对配置`];
...
// 自动执行）/ dsh（宿主原生智能体自动执行）/ 委托开关任一开启，均为委托态
export function stageDelegated(rcx, stageId) {
  ...
  if (stageId === "P6" && (p6m === "session" || p6m === "claude" || p6m === "dsh")) return true;
...
export async function buildDelegateTask(rcx, stageId) {
  ...
  // 输入：trigger=触发原文 / failure=失败信息 / goal=全局目标投影 / 产物路径列表 / repo=本地仓库
  ...
  for (const inp of inputs) {
    if (inp === "trigger") {
      ...
      // 与本机执行同口径：goalBriefOf 投影（含边界声明），不走通用 12K 全文截断
      lines.push(`## 输入 · 全局目标（背景，仅供理解与验收对照）`, "```markdown", goalBriefOf(rcx) || "（01 缺失，无目标投影）", "```", "");
    } else if (inp === "repo") {
...
// P6 必须先有最终报告，再由 verifyDelegateResult 验证完整性与补丁。
export function delegateReady(runDir, stageId) {
  if (stageId === "P6") {
    return delegateReadyFromFile(runDir, "06-implementation/coder-report.json");
  }
  if (stageId === "P11") {
    // P11 的委外契约包含说明与六项 eval；只看到说明文件不能证明 Gate 已完成。
    return delegateReadyFromFile(runDir, "10-pr-description.md") &&
      delegateReadyFromFile(runDir, "11-eval-report.json");
  }
...
export function purgeDelegateArtifacts(runDir, stageId) {
  ...
  if (stageId === "P11") kill("11-eval-report.json");
  return removed;
}
