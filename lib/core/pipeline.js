...
import { newStageExecutionId } from "../infra/ids.js";

// 执行身份匹配（TASK-01）：盘上阶段状态与本轮执行的 stageExecutionId 和 startedAt
// 都一致才算同一轮。旧 run 无 stageExecutionId 时退化为 startedAt 比对（兼容形态）。
function sameExecution(disk, stageId, st) {
  const diskSt = disk?.stages?.[stageId];
  return !!diskSt && diskSt.startedAt === st.startedAt
    && (diskSt.stageExecutionId || null) === (st.stageExecutionId || null);
}
...
// TASK-07：阶段失败的结构化错误对象（可序列化，存入 st.errorInfo；st.error 保持
// 字符串形态供既有 UI/日志消费，新增字段缺省即旧形态，不做数据迁移）。
// 错误来源可能是 LlmError（带 code/finishReason/retryHistory/留档身份）、
// 委外验证或普通异常；统一补齐执行身份，重启后 P10 仍可从 run.json 确定性读取。
export function structuredErrorOf(e, stageId, st = null) {
  const err = e && typeof e === "object" ? e : null;
  const msg = String((e && e.message) || e);
  // 留档引用：LLM 错误带单个 logDir（trace/llm/<callId>/<attemptId>/）；
  // 委外失败带 logDirs 数组（claude 三本 journal / dsh 一本），去重、有界
  const logRefs = [];
  if (typeof err?.logDir === "string" && err.logDir) logRefs.push(err.logDir);
  for (const d of Array.isArray(err?.logDirs) ? err.logDirs : []) {
    if (typeof d === "string" && d && !logRefs.includes(d)) logRefs.push(d);
  }
  return {
    code: err?.code || "stage_failed",
    message: msg,
    stage: stageId,
    stageExecutionId: st?.stageExecutionId || null,
    callId: err?.callId || null,
    attemptId: err?.attemptId || null,
    llmCause: err?.llmCause || null,
    provider: err?.provider || null,
    finishReason: err?.finishReason || null,
    cause: err?.cause ? { name: err.cause.name, message: String(err.cause.message || err.cause), code: err.cause.code || null } : null,
    // 委外执行器自身观测的失败事实（spawn/timeout/result-error/exit/empty/agent），P10 据此确定性分类
    ...(err?.failureKind ? { failureKind: String(err.failureKind).slice(0, 32) } : {}),
    // TASK-10：留档完整性随错误贯通（complete/partial/write_failed/corrupt/open），UI 据此区分「留档不完整」
    ...(err?.logIntegrity ? { logIntegrity: String(err.logIntegrity).slice(0, 32) } : {}),
    logRefs: logRefs.slice(0, 8).map((s) => s.slice(0, 200)),
    // 异常日志保存：错误自身栈轨迹有界留存（仅本机代码路径，不含认证信息）
    ...(err?.stack ? { stack: String(err.stack).slice(0, 4000) } : {}),
    retryHistory: Array.isArray(err?.retryHistory) ? err.retryHistory : [],
...
function appendEvent(runDir, stage, ev) {
  try {
    appendArtifactLine(runDir, "trace/events.jsonl", {
      ...
      ...(typeof ev.stageExecutionId === "string" && ev.stageExecutionId ? { stageExecutionId: ev.stageExecutionId } : {}),
      ...
      // 失败事件携带完整日志引用与留档完整性（与 helpers.logEvent 同口径），从摘要可定位原文
      ...(typeof ev.logRef === "string" && ev.logRef ? { logRef: ev.logRef.slice(0, 200) } : {}),
...
export async function advance(rcx) {
  ...
  for (;;) {
    ...
    if (!nextId) {
      ...
      const delivery = validateDeliveryReport(runDir, { stageExecutionId: run.stages.P11?.stageExecutionId || null });
      if (!delivery.ok) {
        ...
        appendEvent(runDir, "P11", { kind: "stage", name: "P11 交付验收未通过",
          stageExecutionId: st.stageExecutionId || null, detail: st.error, ok: false });
      } else {
    ...
    st.stageExecutionId = newStageExecutionId(nextId); // 每次执行（含重跑）独立身份
    saveRun(runDir, run);
    appendEvent(runDir, nextId, { kind: "stage", name: nextId + " 开始",
      stageExecutionId: st.stageExecutionId, detail: STAGES.find((s) => s.id === nextId)?.name || "" });
    const t0 = Date.now();
    try {
      ...
      if (!diskAfter || diskAfter.status === "stopped" || diskAfter.status === "deleted"
          || !sameExecution(diskAfter, nextId, st)) {
      ...
      if (stageDelegated(rcx, nextId)) {
        if (!delegateReady(runDir, nextId)) {
          ...
          appendEvent(runDir, nextId, { kind: "stage", name: nextId + " 委外产物验证" + (v.ok ? "通过" : "未通过"),
            stageExecutionId: st.stageExecutionId,
            detail: v.ok
      ...
      if (nextId === "P11" && !stageDelegated(rcx, nextId)) {
        const delivery = validateDeliveryReport(runDir, { stageExecutionId: st.stageExecutionId || null });
      ...
      if (!diskAfterError || ["stopped", "deleted"].includes(diskAfterError.status)
          || !sameExecution(diskAfterError, nextId, st)) {
        Object.assign(run, diskAfterError || { status: "stopped" });
        return;
      }
      st.status = "failed"; st.error = String((e && e.message) || e);
      st.errorInfo = structuredErrorOf(e, nextId, st); // TASK-07：结构化错误与字符串并存
      run.status = "failed";
      appendSpan(runDir, { span: nextId, ms: Date.now() - t0, error: st.error });
      appendEvent(runDir, nextId, { kind: "stage", name: nextId + " 失败",
        stageExecutionId: st.stageExecutionId, detail: st.error, ms: Date.now() - t0, ok: false,
        logRef: st.errorInfo.logRefs[0] || undefined, logIntegrity: st.errorInfo.logIntegrity });
      saveRun(runDir, run);
      return;
    }
    saveRun(runDir, run);
