...
// TASK-10：产物绝对路径（下载端点流式发送用；防 .. 逃逸与 readArtifact 同口径）。
// 只解析路径不读内容——大文件按字节流原样传输，读取端上限不得反向变成存储截断。
export function artifactPath(runDir, rel) {
  return safeJoin(runDir, String(rel));
}
