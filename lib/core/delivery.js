...
// TASK-07：验收报告归属校验。报告携带 stageExecutionId（P11 每次执行独立身份），
// 与当前执行不一致 = 旧一轮的历史结果，不得复用其通过状态；无字段的旧报告按旧格式兼容。
export function validateDeliveryReport(runDir, { stageExecutionId = null } = {}) {
  let report = null;
  const errors = [];
  try {
    ...
    if (!text?.trim()) errors.push("11-eval-report.json 缺失或为空，尚未完成验收");
      ...
      try {
        ...
        if (stageExecutionId && report?.stageExecutionId && report.stageExecutionId !== stageExecutionId) {
          errors.push("11-eval-report.json 属于上一轮 P11 执行（stageExecutionId 不匹配），历史通过结果不可复用，请重跑 P11");
        }
        errors.push(...evaluationErrors(report));
      }
