...
async function runClaude({ bin, repoDir, addDir, prompt, timeoutMs, onChild, env, permission, settingsPath, onStdoutChunk }) {
    ...
    return await new Promise((resolve) => {
        ...
        child.stderr?.on("data", (d) => { if (!done && stderr.length < 2e6) stderr += stderrDecoder.write(d); });
...
async function run({ rcx, bin, repoDir, runDir, prompt, timeoutMs, params, auth, logPath, onCapture }) {
    ...
    const stageStartedAt = rcx.run?.stages?.P6?.startedAt;
    if (stageStartedAt && runDir) {
        try { onCapture?.(records.captureId); } catch { /* 观测身份保存失败不能中断执行 */ }
        activity = createAgentActivityWriter({ runDir, captureId: records.captureId, stageStartedAt });
    }
    if (log) {
    ...
    try {
        r = await (rcx.spawnExternal || runClaude)({ // 测试注入点
            ...
            onStdoutChunk: (s) => { streamed = true; if (log && !log.destroyed) log.write(s); records.push(s); },
            onChild: (c) => activeExternals.set(runDir, c),
        });
        if (!streamed && r.stdout) { if (log && !log.destroyed) log.write(r.stdout); records.push(r.stdout); }
    } finally {
        records.flush();
        await activity?.finish(r ? normalizeOutcome(r, auth) : { code: -1, error: "执行器异常退出" });
