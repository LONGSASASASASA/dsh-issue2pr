...
async function run({ rcx, bin, repoDir, runDir, prompt, timeoutMs, params, auth, logPath }) {
    ...
    try {
        r = await (rcx.spawnExternal || runClaude)({ // 测试注入点
            ...
            onStdoutChunk: (s) => { streamed = true; if (log) log.write(s); appendAgentTimeline(timelinePath, s); },
            onChild: (c) => activeExternals.set(runDir, c),
        });
        if (log && !streamed && r.stdout) { log.write(r.stdout); appendAgentTimeline(timelinePath, r.stdout); }
    } finally {
