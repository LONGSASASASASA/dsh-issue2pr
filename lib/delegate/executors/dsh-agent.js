...
async function runDshAgent({ rcx, repoDir, runDir, prompt, timeoutMs, params, onNativeEvent }) {
    ...
    try {
        ...
        // 正常、异常及超时都在 dispose 前采集已发生的原生事件；不把归一 Outcome 伪装成原始消息。
        // followup 前的历史属于别的执行，只有明确的 seq 边界才纳入本轮。
        if (Number.isFinite(firstSeq)) {
            try {
                for (const event of agent.session.events) {
                    if (Number.isFinite(event?.seq) && event.seq >= firstSeq) {
...
async function run(runCtx) {
    ...
    let r;
    try {
        r = await runner({ ...runCtx, onNativeEvent: (event) => {
            nativeCount += 1;
