...
async function runDshAgent({ rcx, repoDir, runDir, prompt, timeoutMs, params }) {
    ...
    const t0 = Date.now();
    let timedOut = false;
    const timer = setTimeout(() => { timedOut = true; try { agent.cancel({ kind: "parent" }); } catch { /* 已结束 */ } }, timeoutMs);
    try {
        await agent.whenIdle(); // 初始静默（官方范式：followup 前必须等 factory 侧 setup 完成）
        const firstSeq = agent.session.seq;
        agent.followup(userMessage(prompt));
        await agent.whenIdle(); // 驱动到静默（turn 完成）
        ...
        await disposeHandle(handle); // 消费者必须 dispose（官方所有权契约）
    }
}
