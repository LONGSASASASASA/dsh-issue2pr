...
//      非空、形如 unified diff；coder-report.json 必须完整覆盖 TaskGraph。
...
import { validateDeliveryReport } from "../core/delivery.js";
...
function validateP6TaskReport(runDir, report) {
  if (!report || typeof report !== "object" || Array.isArray(report)) {
    return ["coder-report.json 必须是任务报告对象"];
  }
  // 兼容旧 patches 报告：每条补丁仍须声明 node，并覆盖全部计划节点。
  // 两套清单同时出现会让验证与 P7 的选取顺序产生歧义，拒绝放行。
  if ("tasks" in report && "patches" in report) return ["coder-report.json 不得同时声明 tasks 与 patches 清单"];
  const tasks = "tasks" in report ? report.tasks
    : Array.isArray(report.patches) ? report.patches.map(task => ({ ...task, status: task?.status ?? "patched" })) : null;
  if (!Array.isArray(tasks) || !tasks.length) return ["coder-report.json 必须包含非空 tasks 或 patches 任务结果清单"];
  ...
  if (graphText == null) {
    errors.push("缺少 05-task-graph.json，无法验证任务是否全部完成");
  } else {
    try {
      const graph = JSON.parse(String(graphText));
      const expected = Array.isArray(graph?.nodes)
        ? graph.nodes.map((node) => String(node?.id || "").trim())
        : [];
      const expectedSet = new Set(expected);
      if (!expected.length || expected.some(node => !node) || expectedSet.size !== expected.length) {
        errors.push("05-task-graph.json 必须包含非空且 id 唯一的 nodes");
      }
...
async function verifyP6(rcx) {
  const { runDir } = rcx;
  const errors = [];
  // coder-report.json 必须存在且可解析（先于清单解析：坏 JSON 在此拦截并给出可定位错误，
  ...
  if (existsSync(reportPath)) {
    ...
    errors.push("缺少 coder-report.json：已有补丁不代表全部任务完成");
  }
  let patches = [];
  if (!errors.length) {
    try {
  ...
  if (!errors.length && !patches.length) errors.push("coder-report.json 未声明任何补丁，P7 无补丁可应用");
...
async function verifyP11(rcx) {
  ...
  const delivery = validateDeliveryReport(rcx.runDir);
  const errors = [...evidence.errors, ...delivery.errors];
  return {
    ok: errors.length === 0,
    errors,
