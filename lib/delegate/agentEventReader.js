...
const INDEX = BASE + "external-exec.events.jsonl";
...
async function originalMessage(file, event) {
...
async function indexedMessage(runDir, id) {
    let index, messages;
    try {
        index = await openSource(runDir, INDEX);
        let event;
        const indexState = {};
        ...
        messages = await openSource(runDir, MESSAGES);
        const message = await originalMessage(messages, event);
        const result = { ok: true, status: "available", event, message, related: [], sourcePath: MESSAGES };
        ...
        const calls = new Set(group.filter(item => item.relation === "call").map(item => `${item.source?.offset}:${item.blockIndex ?? ""}`));
        ...
        for (const candidate of group) {
            ...
            try { result.related.push({ event: candidate, message: await originalMessage(messages, candidate) }); }
        ...
        await index?.close(); await messages?.close();
    }
}
