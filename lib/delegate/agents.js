...
import { resolveClaudeBin, claudeCommonCandidates, authHintOf } from "./executors/claude-code.js";
import { claudeAuthEnv, authRelayActive, writeRelaySettings, cleanupRelaySettings } from "./executors/claude-auth.js";
...
// opts.auth 与 P6 委托同一套认证中转（env + --settings 临时文件）：门禁测的就是真实执行路径。
export function realRunPrompt(bin, opts, cb) {
  const timeoutMs = Number(opts && opts.timeoutMs) > 0 ? Number(opts.timeoutMs) : 120000;
  const settingsPath = writeRelaySettings(opts && opts.auth);
  const args = ["-p", "--output-format", "json"];
  if (settingsPath) args.push("--settings", '"' + settingsPath + '"');
  let child;
  try { child = spawn(bin, args, { shell: true, windowsHide: true, env: { ...process.env, ...claudeAuthEnv(opts && opts.auth) } }); }
  catch (e) { cleanupRelaySettings(settingsPath); return void cb(new Error(String((e && e.message) || e))); }
  let stdout = "", stderr = "", done = false;
  const finish = (r) => { if (done) return; done = true; clearTimeout(timer); cleanupRelaySettings(settingsPath); cb(null, r); };
  const timer = setTimeout(() => finish({ code: -2, timeout: true, stdout, stderr }), timeoutMs);
  child.on("error", (e) => finish({ code: -1, error: String((e && e.message) || e), stdout, stderr }));
...
// —— 认证类错误提示 authHintOf 已迁至 executors/claude-code.js（与 P6 委托失败分类共用，含中转感知） ——
...
// runners 可注入（单测）：{ runWhich, runVersion, runPrompt }；auth 为认证中转配置
// { preset, baseUrl, token }（未保存的表单值也可先测），与 P6 委托同一条真实执行路径。
export async function testAgentGate({ bin = "", runners = {}, timeoutMs = 120000, auth = null } = {}) {
  const R = {
  ...
  // 3) 认证：headless 微任务真实过一遍凭据（403 IP 白名单 / 未登录在此拦截）
  {
    ...
    const r = await new Promise((res) => R.runPrompt(path, { timeoutMs, auth }, (err, out) => res({ err, out })));
    if (r.err) {
      const msg = String((r.err && r.err.message) || r.err);
      steps.push({ name: "认证", ok: false, detail: msg.slice(0, 300), ms: Date.now() - t1 });
      return fail("认证微任务未能执行: " + msg.slice(0, 300), authHintOf(msg, relay));
    }
    const out = r.out || {};
    let ok = out.code === 0;
    let result = "";
    ...
    if (!ok) {
      return fail("claude 认证/调用未通过: " + steps[steps.length - 1].detail, authHintOf(text + " " + (out.timeout ? "timeout" : ""), relay));
    }
    return { ok: true, bin, path, version, steps, message: "门禁通过（" + (version || "版本未知") + " · 认证正常" + (relay ? " · 中转 " + auth.preset : "") + "）", ms: Date.now() - t0 };
  }
}
