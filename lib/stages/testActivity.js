...
const SNAPSHOT_FIELDS = ["version", "status", "captureId", "stageStartedAt", "command", "startedAt", "finishedAt", "updatedAt",
  "executionStatus", "lastOutputAt", "outputBytes", "exitCode", "signal", "outputPath", "outputReady", "logError"];
...
export function createTestActivity({ runDir, command, stageStartedAt, flushMs = 500 }) {
  ...
  const owned = () => {
    ...
    try {
      ...
      sawRunFile = true;
      // pending/新 startedAt 表示已重置；stopped 只停止流水线，不冒充命令已结束。
  ...
  return {
    ...
    async finish({ executionStatus, exitCode = null, signal = null } = {}) {
...
function validSnapshot(value, stageStartedAt) {
  return value && value.version === 1 && value.status === "available"
    ...
    && ["running", "completed", "failed", "timeout"].includes(value.executionStatus)
