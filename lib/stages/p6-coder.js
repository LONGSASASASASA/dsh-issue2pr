...
import { requireArtifact, readRepoFile, logEvent, goalBriefOf } from "./helpers.js";
...
function buildSessionTask(goal, graph) {
  return [
    "# P6 会话任务包（交外部编码会话执行：workflow + superpowers）", "",
    // 委外 agent 可见全仓库、自主性强：完整目标投影 + 边界声明必带（goal 块自带边界行）
    ...(goal ? [goal, ""] : []),
...
async function delegateExternal(rcx, executor) {
  ...
  const prompt = [
    ...
    // 委外自主性强：目标投影 + 边界声明直接进执行 prompt（与任务包同口径）
    ...(goal ? ["- 全局目标（背景，仅供理解与验收对照）：" + goal] : []), "",
    "# 产物契约（流水线据此验收，缺一不可）",
...
export default async function execute(rcx) {
  ...
  const goal = goalBriefOf(rcx); // 决策层（派单）完整投影；执行层（coder）轻量形态——见下

  if (rcx.p6Mode === "session" || rcx.p6Mode === "claude" || rcx.p6Mode === "dsh") {
    writeArtifact(rcx.runDir, "06-implementation/session-task.md", buildSessionTask(goal, graph));
  ...
  // 1) Planner 派单（派单是方向性映射：节点 → 文件，给完整目标投影）
  ...
  // 执行层只给轻量目标（goal 一句 + 边界）：消歧义够用，且不诱发越出派单的扩权
  const coderGoal = goalBriefOf(rcx, { light: true });
  const patches = [];
