...
async function delegateExternal(rcx, graph, executor) {
  ...
  const prompt = [
    ...
    '   {"mode":"' + executor.id + '","patches":[{"node":"T1","patch":"06-implementation/patches/0001-T1.diff","note":"一句话"}],"summary":"总体说明"}', "",
    "完成后只输出一行总结。",
  ].join("\n");
  ...
  const progressTimer = setInterval(() => {
    ...
    if (n !== lastN) {
      ...
      logEvent(rcx, { kind: "tool", name: execLabel + " 进度 " + n + (total ? "/" + total : ""),
...
export default async function execute(rcx) {
  ...
  if (rcx.p6Mode === "session" || rcx.p6Mode === "claude" || rcx.p6Mode === "dsh") {
    ...
    return await delegateExternal(rcx, graph, executor);
  }
