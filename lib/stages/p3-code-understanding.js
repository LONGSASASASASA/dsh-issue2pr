...
import { requireArtifact, readRepoFile, maybeDelegate, goalBriefOf } from "./helpers.js";
...
export default async function execute(rcx) {
  ...
  const goal = goalBriefOf(rcx); // 决策层注入：修改点须按全局目标关联度排序，无目标则只能靠猜
  const md = await rcx.llm.complete({
    system: sysOf(rcx, "P3"),
    user: `${goal ? goal + "\n\n" : ""}【候选文件内容】（每行前缀「行号|」为源码真实行号）\n${files}\n\n【打回意见】${rcx.reviewComment || "无"}`,
  });
