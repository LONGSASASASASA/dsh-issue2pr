...
// patch 清单：优先 coder-report.json（builtin 写 patches 字段；claude 委托写 tasks 字段）。
// 接受相对 runDir、相对 06-implementation/ 及当前 runDir 内的绝对路径，统一返回相对 runDir 路径；
...
export function collectPatches(runDir) {
  ...
  const normalize = (value) => {
    ...
    // C:foo 是依赖盘符工作目录的相对路径；异平台的盘符路径也不能当作普通相对路径拼接。
    if (/^[a-z]:/i.test(raw) && !isAbsolute(raw)) {
      throw new Error("非法路径（patch 盘符路径无法解析）: " + value);
    }
    const patch = isAbsolute(raw) || raw.startsWith("06-implementation/")
      ? raw : "06-implementation/" + raw;
