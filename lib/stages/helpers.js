...
export async function readTriggerText(rcx) {
  ...
  if (/^https?:\/\//.test(uri)) {
    ...
    if (gh) {
      ...
      // 匿名 API 限流仅 60 次/小时/IP，极易 403；本机有 token（gh CLI 登录写入的
      // GITHUB_TOKEN / GH_TOKEN）则带上，提升到 5000 次/小时
      ...
      if (!resp.ok) {
        const hint = resp.status === 404 ? "（仓库不存在或为私有仓库，可改用本地导出的 issue 文档）"
          : resp.status === 403 ? `（API 限流${ghToken ? "（含认证请求）" : "，可设置 GITHUB_TOKEN 环境变量提升额度"}，稍后重试或改用本地文档）` : "";
