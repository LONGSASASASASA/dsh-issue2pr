...
- C2 分两层：先做无依赖权限硬化，再提供 keychain-first SecretStore；权限硬化不得标记为加密。
...
- [ ] 审计现有 CSS/组件并记录需要保留的交互。
- [ ] 引入定稿令牌、明暗主题和 3px 形状体系。
- [ ] 依次重排项目、运行、产物、配置、说明页面。
- [ ] 保留树目录 aria-expanded/aria-controls、焦点样式、空/加载/错误态。
- [ ] 用 Playwright 在 1257x1320、880x1000 和窄屏检查无溢出、无 Console 错误。
- [ ] 运行现有 UI smoke 与 npm test。
...
- [ ] 写出半截写入和 drive rejection 的失败测试。
- [ ] 实现唯一临时文件、写后替换和清理；保留路径穿越校验。
- [ ] 在 drive 顶层记录错误并调用 failRun 兜底，避免锁链吞错。
- [ ] 验证 malformed run.json、P10 失败和 stop/delete 竞态不产生静默 running。
...
- [ ] 添加重复回滚、已变更工作区和超限打回的失败测试。
- [ ] 回滚前执行 git apply -R --check，成功后记录状态，重复请求幂等。
- [ ] 增加默认且可配置的最大复核尝试次数，超过后转 failed/P10。
- [ ] 回归已有 patch apply、rollback 和 reject 行为。
...
- [ ] 先写出 argv 不含 token、文件权限/迁移、patch hash 和实际 diff 校验的失败测试。
- [ ] Git clone/ls-remote 改为 askpass/credential helper 或 header 注入，避免 URL argv 泄露。
- [ ] 实现无依赖权限硬化；JSON 只作为明确标记的 fallback，不宣称加密。
- [ ] 增加 keychain-first 适配边界和旧明文迁移路径；无可用 keychain 时显式降级。
- [ ] ledger 记录每个 diff 的 SHA-256，P11 校验实际仓库变更与已应用 patch 一致。
- [ ] 验证日志、错误和 API 响应无明文 token。
...
- [ ] 写出部分路由覆盖和 git hash 失败的失败测试。
- [ ] 部分 provider/model 覆盖显式报错，不再静默回落。
- [ ] hash 失败使用明确错误/null 契约，禁止 'nogit' 混入真实 hash 字段。
- [ ] 跑 npm test、git diff --check、npm pack --dry-run。
- [ ] 更新审查报告状态和剩余已知取舍；不创建未经确认的发布 tag。
