...
`;
		...
		const aiStore = {
			open: false,
			// 自由窗口矩形（x/y/w/h，相对工作台）：localStorage 快路径 + 服务端 ui-state 兜底；
			// 兼容旧版字段（i2p.aiW/i2p.aiTop）：miss aiX/aiH 时按"右贴、底到页底"推导
			rect: (function () {
				const d = aiDefaultRect();
				const x = clampInt(localStorage.getItem("i2p.aiX"), 0, 4000, null);
				const y = clampInt(localStorage.getItem("i2p.aiY"), 44, 2000, null);
				const w = clampInt(localStorage.getItem("i2p.aiW"), 240, 760, null);
				const h = clampInt(localStorage.getItem("i2p.aiH"), 200, 1800, null);
				const oldTop = clampInt(localStorage.getItem("i2p.aiTop"), 44, 2000, null);
				if (x == null && y == null && w == null && h == null && oldTop == null) return d;
				const p = aiPageRect();
				const w2 = w != null ? w : d.w;
				const y2 = y != null ? y : (oldTop != null ? oldTop : d.y);
				return {
					x: x != null ? x : Math.round(p.w - 78 - w2),
					y: y2,
					w: w2,
					h: h != null ? h : Math.max(200, Math.round(p.h - 16 - y2)),
				};
			})(),
			listeners: new Set(),
			set(v) { this.open = v; for (const fn of this.listeners) fn(); },
			...
			setRect(patch, persist) {
				...
				if (persist) {
					try {
						localStorage.setItem("i2p.aiX", String(x));
						localStorage.setItem("i2p.aiY", String(y));
					...
					fetch(API + "/ui-state", {
						method: "POST",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({ aiX: x, aiY: y, aiW: w, aiH: h }),
		...
		// 默认窗口：右贴（避宿主右缘悬浮按钮排约 70px）、头部下方、底到页底
		function aiDefaultRect() {
			const p = aiPageRect();
			return { x: Math.max(4, Math.round(p.w - 78 - 400)), y: 60, w: 400, h: Math.max(200, Math.round(p.h - 76)) };
		}
		...
		function AssistantDock() {
			...
			// 从服务端 ui-state 恢复（兼容旧 aiW/aiTop）；本地有记忆时反向同步服务端（防旧值空窗）
			React.useEffect(function () {
				apiGet("/ui-state").then(function (r) {
					if (!r || !r.ok || !r.state) return;
					const local = ["i2p.aiX", "i2p.aiY", "i2p.aiW", "i2p.aiH", "i2p.aiTop"].some(function (k) { return localStorage.getItem(k) != null; });
					if (local) {
						fetch(API + "/ui-state", {
							method: "POST",
							headers: { "Content-Type": "application/json" },
							body: JSON.stringify({ aiX: aiStore.rect.x, aiY: aiStore.rect.y, aiW: aiStore.rect.w, aiH: aiStore.rect.h }),
						}).catch(function () { /* 兜底失败静默 */ });
					} else if (r.state.aiX != null || r.state.aiW != null || r.state.aiTop != null) {
						aiStore.setRect({
							x: r.state.aiX, y: r.state.aiY != null ? r.state.aiY : r.state.aiTop,
