...
.i2p-ai-grip-h{top:0;left:0;right:0;height:8px;cursor:row-resize}
...
`;
		...
		const aiStore = {
			open: false,
			// 面板尺寸记忆（宽/面板顶）：localStorage 快路径，服务端 ui-state 兜底跨软件重启
			w: clampInt(localStorage.getItem("i2p.aiW"), 180, 560, 400),
			top: clampInt(localStorage.getItem("i2p.aiTop"), 44, aiMaxTop(), 60),
			listeners: new Set(),
			set(v) { this.open = v; for (const fn of this.listeners) fn(); },
			setSize(patch, persist) {
				if (patch && patch.w != null) this.w = Math.max(180, Math.min(560, Math.round(patch.w)));
				if (patch && patch.top != null) this.top = Math.max(44, Math.min(aiMaxTop(), Math.round(patch.top)));
				if (persist) {
					try {
						localStorage.setItem("i2p.aiW", String(this.w));
						localStorage.setItem("i2p.aiTop", String(this.top));
					} catch (e) { /* webview localStorage 可能被禁，服务端兜底仍在 */ }
					fetch(API + "/ui-state", {
						method: "POST",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({ aiW: this.w, aiTop: this.top }),
		...
		function useAiState() {
			const [s, setS] = React.useState({ open: aiStore.open, w: aiStore.w, top: aiStore.top });
			React.useEffect(() => aiStore.subscribe(() => setS({ open: aiStore.open, w: aiStore.w, top: aiStore.top })), []);
			return s;
		}
		...
		// 面板顶缘上限随视口高：保证面板至少约 200px 高（矮屏下 600 的静态上限会把面板挤成一条线）
		function aiMaxTop() { return Math.max(150, Math.min(600, (window.innerHeight || 720) - 240)); }
		...
		function AssistantDock() {
			...
			// 面板尺寸兜底：宿主 webview 的 localStorage 不跨软件重启——本地无记忆时
			// 从服务端 ui-state 恢复；本地有记忆时反向同步服务端（防旧值空窗）
			React.useEffect(function () {
				apiGet("/ui-state").then(function (r) {
					if (!r || !r.ok || !r.state) return;
					const local = localStorage.getItem("i2p.aiW") != null || localStorage.getItem("i2p.aiTop") != null;
					if (local) {
						fetch(API + "/ui-state", {
							method: "POST",
							headers: { "Content-Type": "application/json" },
							body: JSON.stringify({ aiW: aiStore.w, aiTop: aiStore.top }),
						}).catch(function () { /* 兜底失败静默 */ });
					} else if (r.state.aiW || r.state.aiTop) {
						aiStore.setSize({ w: r.state.aiW, top: r.state.aiTop }, false);
					}
				}).catch(function () { /* 兜底失败静默 */ });
			}, []);

			// 面板尺寸拖拽（仿目录列拖宽）：原生事件委托（宿主注入环境里 React 合成
			// 事件对 pointer 不可靠）；左缘调宽、顶缘调高，双击任一手柄恢复默认
			React.useEffect(function () {
				const gripOf = function (t) { return t && t.closest ? t.closest(".i2p-ai-grip-w,.i2p-ai-grip-h") : null; };
				const down = function (e) {
					const grip = gripOf(e.target);
					if (!grip || e.button !== 0) return;
					const panel = grip.closest(".i2p-ai-panel");
					const page = grip.closest(".i2p-page");
					if (!panel || !page) return;
					e.preventDefault();
					const rect = panel.getBoundingClientRect();
					const pr = page.getBoundingClientRect();
					const isW = grip.classList.contains("i2p-ai-grip-w");
					document.body.classList.add("i2p-dragging");
					if (!isW) document.body.classList.add("i2p-dragging-r");
					const move = function (ev) {
						if (isW) aiStore.setSize({ w: rect.right - ev.clientX }, false);
						else aiStore.setSize({ top: ev.clientY - pr.top }, false);
					};
					const up = function () {
						window.removeEventListener("pointermove", move);
						window.removeEventListener("pointerup", up);
						document.body.classList.remove("i2p-dragging", "i2p-dragging-r");
						aiStore.setSize({ w: aiStore.w, top: aiStore.top }, true); // 拖动结束落盘（双写）
					};
					window.addEventListener("pointermove", move);
					window.addEventListener("pointerup", up);
				};
				const dbl = function (e) {
					if (gripOf(e.target)) aiStore.setSize({ w: 400, top: 60 }, true);
				};
				document.addEventListener("pointerdown", down);
				document.addEventListener("dblclick", dbl);
				return function () {
